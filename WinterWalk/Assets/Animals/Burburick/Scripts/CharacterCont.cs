...
public class CharacterCont : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        // transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(movement), Time.deltaTime );//ѕоворот персонажа в сторону движени€
        // anim.SetFloat("speed", Vector3.ClampMagnitude(movement, 1).magnitude);// ѕлавное ускорение анимации персонажа от 0 до 1
        //Vector3 moveDir = Vector3.ClampMagnitude(movement, 1).normalized * speed;
        //rigidBody.velocity = new Vector3(moveDir.x, rigidBody.velocity.y, moveDir.z); // ”скорение модели персонажа
    }
